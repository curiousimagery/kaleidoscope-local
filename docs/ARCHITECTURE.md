...
**⚠️ THE TWO CHROMES SHARE NO `env`, AND THAT IS THE SINGLE LARGEST SOURCE OF SILENT BUGS IN THIS CODEBASE.** `main.js` and `mobile/chrome.js` each construct their own `env` and wire their own controls, so **a helper added to one simply does not exist in the other.** Seven bugs in the B609-B628 arc came from this, four of them found only by a device session or a live show, because each was invisible on the machine it was written on. The rules that follow are not style preferences; each one is a postmortem.

- **A function injected into shared code must take everything it needs as arguments.** A callback that closes over one chrome's `state` is not shareable. The chrome whose shape matches keeps working, so the failure appears only on the other one — and usually silently, since these paths are commonly guarded. **B627:** `resetSliceState` called an injected `applyArmsSnap()` with no arguments; that suited the desktop wrapper, threw in the mobile chrome, and the iPhone quietly never centred its slice for four builds.
- **Never name a local wrapper after the shared function it wraps.** Identical name plus different signature is what makes the above invisible in review. Suffix it (`applyArmsSnapLocal`) and pass shared code the shared function.
- **Prefer moving a shared behaviour into `kit/` or `engine/` over writing it twice.** `kit/pan.js` (one pan gain for touch, remote and bus) and `engine/geometry.js`'s `resetSliceState` (one slice reset for both chromes) are the pattern.
- **The input bus is desktop-only.** `createInputBus` is called once, from `main.js`. MIDI/gamepad/remote mappings do not reach the phone chrome; do not assume a new mapping target does.
- **A fix verified on desktop is not verified.** Grep the other chrome for the same behaviour before calling it done.

**The repeatable audit** (run it when the shared surface changes): diff the `env` keys each chrome assigns; diff the module import sets; for each shared component diff which `ctx` keys each chrome supplies; and **compare call ARITY for every function exported by a shared module against both chromes' call sites.** The arity check is the one that catches signature traps.

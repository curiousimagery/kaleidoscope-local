# handoff — builds 705-737 (the memory-ceiling arc)

**Archived at B737.** This is the phase-2 narrative from the GL-loss instrument (B705) through the
streaming demux (B737), newest-first, exactly as it was written build by build. It was 780 of
`HANDOFF.md`'s 809 lines and had accumulated eleven "SUPERSEDED" blocks — the same drift Daniel
called out at B704.

**Everything still live was moved out first.** Current state is `../HANDOFF.md`; open work is
`../BACKLOG.md`; the sequence is `../PLAN-LIVE-READINESS.md`; the tests are `../VERIFY-QUEUE.md`.

**What this arc established, in one place, so the detail below is optional reading:**

| finding | build |
|---|---|
| `watchGLContext` — one watcher, five surfaces, four distinguishable outcomes | B705 |
| Four instances of *"a recovery path that cannot start itself"* | B703, B706, B708, B709 |
| A hole in the presentation timeline was a state `frameAt` could not leave | B721 |
| GL loss is provokable on demand, and a provoked loss needs `restoreContext()` | B723, B724 |
| Native thermal/memory only arrived while broadcasting — `load()` was never called otherwise | B726 |
| `footprintMB` is the HOST process; the bake dies in the WKWebView content/GPU processes | B727 |
| The allocation ledger, and `host_statistics64` for a device-wide reading | B728, B729 |
| **Cost is a property of the job; the ceiling is a property of the device** (`peakMB` identical on four machines) | B730-B731 |
| Photos hands out a lower-bitrate re-encode — "same clip" was wrong twice | B731 |
| One shared fetch + demux for slice's two readers: 2143MB → 1441MB | B732 |
| A restored context is not a restored screen (`onGLRestored`) | B733 |
| The muxer streams to disk-backed Blob parts, Fast Start preserved | B734, B736 |
| The demux parses the moov only and indexes into the original Blob | B735-B737 |

**Five of my own instruments were wrong during this arc** (B716 wrong noun, B720 ranking, B722
late-read and dead counter, B727 wrong process, B730 phantom term). The recurring shape is **reading
a value at the wrong moment or from the wrong subject**, not measuring the wrong thing.

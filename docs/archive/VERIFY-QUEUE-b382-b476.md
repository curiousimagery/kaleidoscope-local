# ARCHIVE — verification queue, Builds 382–476

Split out of `docs/VERIFY-QUEUE.md` at Build 547 (2026-08-07).

**Why.** The queue had grown to 165 lines, of which the live matrix was the top 55. The remaining 110 were sediment from three closed arcs — Loop Builder, the M3 lock pass, and the NDI/HDMI wave — mixed together with items already marked ✅ confirmed, including one that said in writing *"Left here as a record; can drop next edit."* A test queue that buries its live section under a year of history is a test queue nobody can work through.

**The Loop Builder items (B385–B406) are obsolete as written, not merely old.** They were authored across fifteen builds of rapid iteration in which the surface was repeatedly reshaped: a mode-menu entry became a full-screen stepped surface (B386), then an editing mode with the app bar visible (B387), then a modal with its own header and step rail (B404), and the step sequence itself was merged and renumbered (B395). Verifying them literally would mean checking an interface that no longer exists — several name buttons, steps and layouts that were removed by a later build in the same run. **They were replaced by a single fresh Loop Builder pass in the live queue**, written against the surface as it actually ships and against the specific defects Daniel believes were fixed.

**What was carried forward to the live queue instead of archived:**
- B382 external-display / GL-context-lost cluster (still open, still reproducible)
- NDI HD-vs-FHD over WiFi (blocked on location, not on the build)
- Large-clip HDMI/AirPlay staging, long-clip half (short clip confirmed)
- Movink resolution + aspect-padlock re-verify (fixed B476, never re-checked on device)
- The `[fold] aspect hard-locked` stale-flag reading

Everything below is kept for reasoning, not for execution.

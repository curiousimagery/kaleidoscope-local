...
4. **Loop Builder (clip-editor UX)** ← NEARLY DONE, arc's closing focus. The full stepped mode shipped and iterated across B385–B395: stepped progressive disclosure, FCP-style crossfade editing (proportional timeline, tap-to-select clip/crossfade, drag endpoints with live seam-match split, freeze-then-reflow), undo/redo, portrait-bake fix, bounce fast-decode + measured-fps bake, output-format settings (resolution/fps/slomo + warnings), merged Trim & behavior step, ruler scrubbing. **Remaining to close the arc:** Daniel's device/desktop verification pass + the two parked gnarly bugs + Safari crossfade-preview stall. See VERIFY-QUEUE + BACKLOG.
5. **App-wide mode-transition guardrails — DEFERRED to BACKLOG (Daniel, 2026-07-20)**, not part of closing this arc. Opinionated Still/Motion/Perform/Loop-builder routing (keyframe-shift warning shipped B386; file-open routing, simplified non-loop motion editor, bounce PLAYBACK mode remain). Picked up as its own arc after Loop Builder closes. See BACKLOG "Mode transitions."

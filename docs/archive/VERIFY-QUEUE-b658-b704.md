# verification queue — closed session, B658-B704

> **📦 ARCHIVED 2026-08-21 (B704).** The session below asked *"where is the ceiling, and is it a
> number we can compute?"* **It is answered.** Every T-item ran; the headline results are in
> `CHANGELOG.md` build by build, and the surviving conclusions live where they get read:
> the exit-criterion run in `PLAN-LIVE-READINESS.md` "Where we are", the delivery facts in
> `BROADCAST-DELIVERY.md`, and the session accounting in `archive/SESSION-AUDIT.md`.
>
> **Nothing here is a to-do.** Kept because several items carry the *reasoning* and the
> superseded framings, which is the part CHANGELOG does not preserve.

## how it came out

| item | question | outcome |
|---|---|---|
| **T2** | does a long 4K→4K HDMI broadcast hold? | ✅ **Answered 2026-08-18.** The collapse is interaction-driven; the broadcast itself is stable |
| **T3** | is a bad take a fixed cost, or a lost contention? | ✅ **Answered.** Automated as `T3 · recording priority A/B` (B665); re-run as **T3r** at B698, completed 19/19 with no context loss |
| **T7** | does anything degrade over 40 minutes? | ✅ **Answered 2026-08-19.** Nothing degrades. The heat hypothesis died here |
| **T8** | can the iPad hold power while broadcasting? | ✅ **Answered 2026-08-19.** Power is solvable — the cable has to go into the iPad |
| **T9** | does a long clip behave differently from a short one? | ✅ **Answered 2026-08-19.** Fine, and better on the wall |
| **T10** | is there a WIRED setup that works? | ✅ **Answered 2026-08-21.** Apple A1621 dongle with power, 50 minutes, `outcome: complete`. **This is the run that met the arc's exit criterion.** 8 loop wraps, 6ms worst gap |
| **T11** | take baseline on the M1 Air | ✅ **Ran 2026-08-21** (B698 scenario) |
| **T1 / T4 / T5** | 4K take w/o broadcast · M1 Air · M1 Max desktop | 🅿️ Repositioned rather than run — see the note at the bottom |

...
**TIER 1 DEVICE VERIFICATION (Daniel, 2026-07-16):** record improved as predicted — **iPad 15→23–25fps**, **Electron NDI 50–60fps**. Two things didn't move as expected, both now understood:

- **iPad NDI regressed 25.5→19.5fps.** The JS side got faster, but readPixels frames are bottom-up, and the frame socket's NATIVE side row-flipped every frame — a new full-frame CPU pass on the socket's DRAIN. The faster loop also floods the socket, exposing the drain's true ceiling that slow production used to hide. **B364 fix:** the flip moved into the JS publish copy (free — that copy already happens per frame); wire is always top-down. If fps doesn't recover past ~25, the next drain suspect is the WebSocket transport itself: client→server frames are mandatorily MASKED (RFC 6455) — two full-buffer XOR passes per 8.3MB frame that a raw TCP NWConnection with the same FNDI framing would eliminate (filed in BACKLOG).
- **Safari desktop recording is still 5–10fps** despite the 45.5→2.7ms readback. The recorder rides `captureStream`+MediaRecorder off the blit canvas — WebKit's encoder path is the wall there, not the readback. That is Tier 2's whole thesis; no plan change.

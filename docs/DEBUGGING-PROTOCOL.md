...
- **Ask him to set the scenario tag explicitly, in the steps, every time.** Daniel (B579): *"in your verification steps, you can help by explicitly asking me to set the tag before I save a report to share and even to save a baseline when helpful, so we're not relying on my fallible memory."* Two B578 reports and a saved baseline landed in the wrong slot for want of one line in the instructions. **A step the instructions do not contain is a step that will not happen.**
- **A paint counter hiding an arrival problem is now a STANDING suspicion, not a discovery.** B519 (`refresh` costing 1.13ms while zero frames arrived), B552 (51fps drawn, 4 new/s), B577 (26 drawn, 7 new pictures). Each was fixed one layer down and reappeared one layer out. **Whenever a rate looks healthy and the picture does not, assume this shape first and go looking for the arrival counter one level further out.**

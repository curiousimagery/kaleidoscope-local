...
Seeded 2026-06-17 during Fold Live Phase 0, from the brainstorm of the full output-controls suite. The inventory below is fairly complete; the per-platform matrix is partially filled (outputs/inputs are confident) and grows as increments land. The **docked output row + traffic-light** in "Locked UI decisions" shipped in Build 177 (Increment 3.5). **Status key:** ✅ done · 🔜 in the current Fold Live increment sequence · 📋 planned (later phase) · 💭 speculative (not committed).
...
- **Output controls = a docked, solid output ROW** that expands down directly under the global bar when the output button is toggled, collapsed and out of the way otherwise (the motion-footer pattern, top-docked). NOT a floating overlay or modal — it does not cover the preview. Holds, at single-click density: the record take toggle (red), the broadcast arm toggle (green; hidden when no live channel), the resolution tier picker, and the Syphon name field.
...
- **Canvas controls relocation:** `frameAspect`, OOB, canvas zoom, canvas rotation are **composition-global**, NOT slice settings. Desktop home: a **canvas button beside motion** in the global bar opening a dropdown with the existing controls. Mobile: a **settings button opposite the flip-camera control** in the output panel (mobile still groups these with slice today; this resolves the inconsistency). Output resolution stays in the output panel; still-export resolution stays in the export sheet; both derive from the composition-global `frameAspect`.

...
`v0.5.11 · Build 85`. The footer in the running app shows this string from `src/version.js`.
...
3. **Multi-keyframe timeline footer (desktop chrome). ← NEXT.** Generalize the A/B pair to a keyframe list; the footer becomes a scrubbable timeline with draggable markers (add/remove/move, possibly uneven), duration, fps, aspect-crop for output framing — keeping the loop-close semantic (last→first, not ping-pong). **IxD-sensitive — Daniel drives the timeline UI design.** The motion data model (`motion` in `state.js`) and the playback driver are built to extend from 2 to N.

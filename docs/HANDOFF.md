...
`v0.7.6 · Build 102`. The footer in the running app shows this string from `src/version.js`.
...
- **Build 102 (shipped, needs Daniel's on-device pass — A2):** PWA tab-bar bottom anchoring, first iteration. Bottom padding trims the home-indicator inset by `--m-tab-clear` (14px, clamped ≥6px) so the bar sits closer to the true bottom; edge buttons round their bottom-outer corner by `--m-tab-corner` (20px). Both are `:root` vars in `src/mobile/styles.css` for fast tuning — **tell me how the clearance/radius feel on-device and I'll adjust.** Open question: full-bleed (drop the 8px side padding) so the corners truly hug the device — deferred to the landscape build's IxD checkpoint since the bar's edge behavior changes there anyway. **Still queued in A2 territory but NOT yet done:** the landscape build itself (carved out, checkpoint-first).

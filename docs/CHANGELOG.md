...
- **Crossfade-timeline UX rework (Daniel's refinement of point 5).** The crossfade band is back to **full height and prominent** (it's the main thing on this step) with directly-draggable edges. Clip endpoint editing moved OUT of its way: **tap a clip** → a highlight appears under the timeline and a **seam bar** rises through the seam and ~28px below the track; drag that bar (from the seam or from underneath) to adjust the clip's edge. **Tap the same clip again, or tap the crossfade, to deselect** — there's a clear way back to the unselected state. Scrubbing works throughout (tap = select, drag = scrub).
- **Output format settings (new — on the Preview & bake step).** Resolution (match source / 4K / 1080p / 720p, downscale-only), frame rate (match source / 24 / 30 / 60), and playback speed (100 / 50 / 25% slomo), applied at bake, with a live spec readout ("1920 × 1080 · 30 fps · 8.0s loop · 25% slomo"). Speed just stretches the bake duration; smooth slomo needs source-fps headroom (120→25% is clean; 30→25% judders until frame interpolation lands — filed, now coupled to this control).
- **Docs:** ARCHITECTURE.md refreshed — the browser-rendering section now describes the runtime adaptive-readback probe (not the old hardcoded per-browser paths), the Loop Builder + WebCodecs bake, and a current conduit module inventory (Tier A/B/C).

Verified: node --check ×3, vite build. **Untested by Claude — desktop + iPhone-portrait verify (VERIFY-QUEUE.md).**

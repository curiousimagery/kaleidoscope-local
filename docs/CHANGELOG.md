...
## v0.10.16 (Build 198) — 2026-06-19 — non-swapped preview centers vertically

**Bug (Daniel, cross-browser): the kaleidoscope preview pinned to the TOP** of the main slot instead of centering when the comp is wide (16:9), leaving dead space below. Firefox + Brave honored the aspect but top-floated; Safari additionally didn't honor the wide aspect at all (separate, still-unexplained symptom — see HANDOFF). Daniel's two screenshots are a clean A/B that isolated the cause: the **swapped** branch (which centers correctly) sizes its wrap `width/height:100%` and flex-centers the canvas, while the **non-swapped** branch used `max-width/max-height:100%`, so the wrap shrank to the canvas and the vertical centering had no room to act. [main.js](../src/main.js) `arrangeSlots` non-swapped branch now fills the stage (`width/height:100%`), matching the proven swapped branch. Build green, `node --check` clean. **Verify (Daniel):** wide comp, output in the main slot → preview centers vertically in Firefox/Brave (Chromium). The Safari "doesn't honor the aspect at all" symptom is tracked separately — need to know if it's swap-specific or Safari-specific.

...
- **Phase 4 — Video export + video-file loops.** **Video export SHIPPED (Build 96, v0.7.0):** WebCodecs `VideoEncoder` → mp4-muxer (H.264 .mp4), frame-by-frame via `engine.exportFrame` with non-square aspect (1:1/4:5/16:9), resolution (1080p/1440p/4K, FBO-capped), fps (24/30/60). **Video-export follow-ups:** MediaRecorder fallback (non-WebCodecs browsers), 3K/6K tiers (memory/codec permitting), aspect crops for *still* export (the engine `u_outputAspect` now supports it). **Still ahead — load video loops as a source** (`<video>.src` = file) reusing the Phase 0 video plumbing; the timeline becomes media-bound; loop-lock matches first/last-frame params for looping source video.

...
- **Zoom interaction model — UX-research reframe (2026-07-28, pending Daniel confirm).** History review (B454 unified-zoom trap fix → B455 slice-cap 3→5 → B459 invertible pivot) shows the *flow* (a single output pinch spilling slice→canvas) was an intentional, well-liked BEHAVIOR CHANGE that fixed a real trap — NOT a bug. So the earlier "decouple by panel" plan (output pinch = canvasZoom only) is essentially a **revert to the pre-B454 trap-prone state** (a wider range only lowers the wall). The genuine best-of-both is a **re-tune, not a decouple**: keep the flow but make it **canvas-PRIMARY** (output pinch drives `canvasZoom` in the normal range, leaving the user's `sliceScale` untouched → small-slice + mega-repeat works by construction; slice-shape stays controllable at zoom-out) with **slice as a BOUNDED overflow at the walls** (grow only to "covers source", not 5× → no unwieldy extreme, no trap). Zoom-in still magnifies canvas first (B459 intent). Implementation = reorder `applyUnifiedZoom`'s 4 segments to canvas-first + change the slice cap from `Z_SLICE_MAX` to a per-form "covers source" value (Phase B calibrates it). Pairs with the overlay density-LOD (tame the reflected copies past N repeats — the honest radial overlay B453 is what "goes crazy" at low canvasZoom, independent of the model).

...
- **[BUG, NEW — Daniel field test 2026-07-28] iPhone HDMI: the ASPECT-LOCK unlock is totally unresponsive.** On the mobile chrome with HDMI out, unlocking the aspect lock does nothing (not just finicky — dead). Daniel's read: "the fix to segment lock didn't make it over" — the recent mobile lock-toggle touch-responsiveness fix (B426–B428 lock gating) reached the segment lock but not the aspect-lock toggle. Audit the aspect-lock toggle against the segment-lock handler (likely the same `touch-action` / layered-tap-capture / `padDown` bail that B429/B430 applied — a missed surface).
...
- **[BUG, NEW — Daniel field test 2026-07-28] iPad NATIVE HDMI detects the wrong display resolution.** Reports the HDMI sink as **2560×1440 for a true 4K display**. With 4K output selected, the *output* reports 3840×2160 correctly, but the **TEST PATTERN renders at the detected mode (2560×1440), not the selected output res** — so it's sent under-res. Contrast: iPad **AirPlay** detects source resolution correctly + shows the right test pattern, so this is HDMI-mode-detection-specific (the plugin reads the wrong `CADisplayMode` / picks a non-native mode). Fix: query the display's native/max mode, and render the test pattern at the SELECTED output res, not the detected mode.
- **[BUG, NEW — same test] External-display gate wrongly blocks MOTION-FROM-A-STILL.** Playing a motion (keyframe) animation whose SOURCE is a still image throws "video sources on the external display are coming — use a still or live camera for now." But a still-sourced motion has no video blob (it's animated params over a still) — the cross-webview blob problem below doesn't apply. The gate keys on "motion mode active" instead of "source is a VIDEO"; loosen it to allow still/camera-sourced motion and only block genuine video sources.

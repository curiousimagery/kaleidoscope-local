...
- **Phase 3 — Tween/easing kit + still-animation.** Tween primitive SHIPPED (Build 82). A/B loop motion mode SHIPPED (Build 83). **Multi-keyframe timeline CORE SHIPPED (Build 87):** N-keyframe data model (`motion.keyframes`), timeline track with saved-state thumbnails + scrubber + loop bookend, add/select/delete, play/pause/loop/stepper, total duration, explicit select-to-edit write-through, discrete locked to kf0. Output toolbar (Build 86) homes undo/swap/motion. Build 88 added: + keyframe inserts-after (never overwrites) with even auto-spacing, sequential edit→add flow (add leaves nothing selected; marker-click = write-through edit), monochrome centered notches + white scrubber, settings gating (non-animatable controls dimmed/disabled in motion mode). **Fast-follows for the timeline:** drag-to-retime markers (and make even-spacing non-destructive once it exists — sequential adds currently re-even the whole set), **motion smoothing** (global control + per-keyframe ease-in/out — the per-segment easeInOut can feel jarring at high playback speed; do it as a proper easing model, likely spline-based, not a linear↔ease blend), pinch-zoom/pan + scale-to-fit, previous/next PiP output monitors (held loosely — large track thumbnails may suffice, esp. iPad portrait), per-segment rotation winding (+N turns), per-marker contextual menu.
...
- **Phase 4 — Video export + video-file loops (prioritized UP, ahead of gesture-record).** First a **video-export host module**: render each interpolated frame via the FBO path (like `exportAt`) into WebCodecs `VideoEncoder` (mp4/h264) with a MediaRecorder fallback (webm/vp9). Then **load video loops as a source** (`<video>.src` = file) reusing the Phase 0 video plumbing; the timeline becomes media-bound; loop-lock matches first/last-frame params for looping source video.

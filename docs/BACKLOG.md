...
- **HIGH PRIORITY — first-keyframe lag (motion mode, reported v0.7.3).** After saving the FIRST keyframe, the "+ keyframe" button is unresponsive for ~4–8s, then recovers; the second keyframe can't be added until then. NOT the filmstrip (fixed Build 99, and the filmstrip skips builds below 2 keyframes so it can't be the cause with one keyframe). Suspect a one-time blocking GPU op on the first add — most likely the `drawImage(previewCanvas …)` snapshot in `makeThumb`/`drawThumbInto` forcing a GPU readback/sync, or the first FBO render; possibly source-size dependent. Investigate with per-step timing; likely fix: make/defer the thumbnail capture async or capture from a small offscreen render instead of the large preview canvas.
...
- **Phase 4 — Video export + video-file loops.** **Video export SHIPPED (Build 96, v0.7.0):** WebCodecs `VideoEncoder` → mp4-muxer (H.264 .mp4), frame-by-frame via `engine.exportFrame` with non-square aspect (1:1/4:5/16:9), resolution (1080p/1440p/4K, FBO-capped), fps (24/30/60). **Video-export follow-ups:** global output aspect (WYSIWYG non-square preview) SHIPPED (Build 98, v0.7.2) — applies to still + video export. **Companion "how it was made" video (delighter):** an opt-in checkbox in the export sheet to also download a second video of the SOURCE with the moving wedge overlay — same fps, square, capped ~1920×1920 (usable as 1080p in either orientation), no extra settings. **Also:** MediaRecorder fallback (non-WebCodecs browsers). (Resolutions 1080p/2.5K/3K/4K/6K SHIPPED Build 97; bitrate auto ~0.1 bpp. Aspect-control-in-footer + offscreen async filmstrip SHIPPED Build 99.)
  ...
  **Video export — perf + robustness (from Daniel's 0.7.x desktop/iPad testing):**
  1. **Render speed / multi-core.** The export loop is serial render→readPixels→encode on the main thread; the per-frame GPU→CPU `readPixels` is the bottleneck (~1 core; "Safari Graphics and Media" ~100%; a 30s 4K@30 took several minutes even from a small source). Faster path: render each frame to a sized canvas and build the `VideoFrame` from the canvas directly (skips readPixels + the Y-flip), and/or move encoding to a Worker.

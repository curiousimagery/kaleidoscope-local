...
- **Timeline thumbnails — blue color shift in Safari only (not Firefox).** The filmstrip/keyframe thumbnails (rendered via `engine.exportFrame` → FBO `readPixels` → `putImageData`) show an unexpected blue tint on *some* frames in Safari; Firefox renders them correctly. Suspects: a premultiplied-alpha / canvas color-management mismatch in the WebGL-readback→2D-canvas path (context is `premultipliedAlpha:false`; `putImageData` expects straight alpha — Safari may treat it differently), OR a race between concurrent `engine.exportFrame` calls sharing one GL context (filmstrip build overlapping keyframe-thumb fills) producing occasional bad reads. NOTE: the video-export perf rewrite (render-to-canvas + `VideoFrame`, skipping `readPixels`) may change/retire this exact thumbnail path — reassess after that lands; if thumbnails move to the same canvas-capture path the tint may disappear.

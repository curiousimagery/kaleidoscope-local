...
- **✅ DIAGNOSED + FIXED B471.** Daniel's ethernet A/B (WiFi off = smooth in Arena, WiFi = halting on BOTH iPad + iPhone) confirmed WiFi-TRANSPORT-bound, not pipeline. Fix SHIPPED: **adaptive publish-rate governor** in `ndi-sink.js` — AIMD pacing to an even sustainable gap (ethernet→gap~0 full rate; WiFi→settles under capacity, smooth). Breadcrumb `[fold] NDI paced <fps>fps · gap <ms>`. **Pending WiFi device verify.** **Tails if WiFi still imperfect:** tune GAP_UP/DOWN/PROBE_OK; UYVY chroma-subsample further; fetch-POST unmasked transport. Ethernet remains the max-fps rig answer.
...
- **✅ VIDEO SOURCES over the external display — SHIPPED B470 (iPad/desktop chrome; NEEDS DEVICE VERIFY).** Native `stageVideo` (base64 clip → cache file) + `fold-ext://.../staged/*` served with HTTP RANGE (206 — WKWebView `<video>` requires it) + `clearStaged`; JS `buildSourcePayload` returns `{kind:'video',url}`. Render + program-clock sync (`reconcileVideo`) already existed. ~60MB cap (base64 jetsam guard). **Tails:** (a) device-verify playback + sync; (b) large-clip path — base64-over-bridge is the cap driver; a localhost-socket transfer (NDI-style) would lift it if long video sources matter; (c) mobile chrome has no video sources so it's untouched (image+camera only).

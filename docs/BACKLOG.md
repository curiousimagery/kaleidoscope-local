...
- **⚠️ Display-resolution detection is per-device-quirky — DIAGNOSTIC shipped B475, rule TBD from data.** B465 switched to the *largest* `UIScreenMode` (fixed the 4K adapter that `bounds × scale` under-reported as 1440). But Daniel's **Movink 13 reports a QHD mode yet is physically FHD** — "largest" over-reports there. No single naive rule is right for both. B475 adds a one-shot `[FoldExt] display modes` log (preferred / bounds×scale / picked / all modes) — Daniel reads it off the Movink (and re-confirms the 4K adapter later), then we pick a provably-correct rule (likely `preferredMode` with a sanity fallback). **Output resolution was always correct — only the reported identifier is wrong**, so this is low-pri QoL. `nativeSize`/`applyNativeMode` still pick largest until the rule lands.

...
- **[✅ SHIPPED B653 — AWAITING DANIEL'S VERIFICATION, THEN DELETE THIS LINE] Perform's timescale ruler isn't scrubbable.** The footer timeline (`#srcScrub`) scrubs; the ruler above it (`#pfRuler`) was decorative — not one listener, while the `.mf-ruler` class it shares with motion already declared `cursor: pointer; touch-action: none`. B653 wires pointerdown/move/up on it, seeking `env.sourceClock` in the trimmed + retimed frame the ruler labels. **▶ B654:** Daniel verified it functions but *"is very slow... scrubbing via the ruler pauses a beat."* Cause: B653 wrote its own `clock.seek()` per pointermove instead of calling `scrubStillFrame`, which coalesces latest-wins, uses `seekSettled` + `refreshFrame` on the native path, stands down the thumb pass and repaints synchronously. Both surfaces now call the shared `env.scrubSourceTo(p)`. **Daniel's instruction: remove this entry as soon as the fix is verified.**

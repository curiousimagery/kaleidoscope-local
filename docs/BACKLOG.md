...
- **[DIRECTED — Daniel 2026-07-08, SPEC PROPOSED, awaiting his react] "Stage changes" — live keyframe editing while a motion animation plays.** His framing: not merging motion+perform; a scoped staging state inside motion, ghost-differentiated edits, then a take/commit that lets the running loop transition onto the new keyframes seamlessly. **Proposed IxD (mirrors perform's stage/take/cut vocabulary + machinery):**
  ...
  5. **A slim staging bar** (over the ruler's right end, not a new footer row): "staging · N changes" + [take] [cut] [discard]. Keyboard: T = take, S = exit-staging toggle (prompts if changes exist), space stays play/pause (motion semantics untouched).

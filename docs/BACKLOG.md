...
- **[MED] Motion mode: canvas/form settings in the popovers stay live during playback (should lock).** Intent (confirmed with Daniel): keyframe params LOCK during motion playback — pause to adjust, then save a keyframe. But the `#canvasPopover` / `#slicePopover` sliders + scrub fields never consult `editLocked`/`isMotionDriven` (controls.js has no such check), so during playback they move + write live state which the motion tick clobbers next frame (the "moves but doesn't save anywhere" Daniel saw). **Fix seam:** `updateMotionUI()` (motion-runtime.js, fires on every play/stop) toggles a lock class on the popovers; CSS makes the control rows inert — mirror the overlay/gesture `editLocked` guard. Decide scope: all form+canvas controls (out-of-bounds already special-cased via `data-tip-motion`) + a dim / "paused to edit" affordance. NOTE: the B445 offset joysticks already respect the lock via their own `locked()` guard; this is the remaining slider/scrub surface.
...
- **Unlockable "repeating movements" — GREENLIT PLAN (Daniel, 2026-07-25).** The lock unblocks a class of default-locked live-driveable motions. Model Daniel set: **manual controls in canvas settings + matching canvas gestures**; autoplay / direction-lock / recenter are *fractional polish on top of a working core loop*, not core. Priority order + status:
  ...
  - **③ Offset + radial translation via the SAME joystick — 🚧 DESKTOP SHIPPED B445.** Reuse ②'s velocity joystick (now `rowId`/`label`/`locked`-parameterized) for two non-looping cases: **radial** ungates the existing canvasOffset joystick (translates the center, `periodOf`→null); **droste** gets a 2nd instance driving `drosteOffsetX/Y` (the Möbius center), mounted in the `#drosteOffsetLabel` row, DISABLED unless `manual` is on (the same unlock that gates the diamond drag — keeps the offset centered by default, which seamless infinite zoom depends on) or while an animation drives state. Both respect the motion edit-lock via `locked()`. **STILL OPEN:** (a) **mobile droste-offset joystick** — deferred: the mobile chrome has no `manual`-gate UI to port yet (radial pan IS ungated on mobile). (b) radial-center via `canvasOffset` translates the whole plane; if Daniel wants "move the sampling center on the source" instead, that's a different mapping — get on-device read. (c) position DOT is centered (no feedback) in the non-looping case — could show absolute offset clamped. (d) NOT wired to a gesture yet (joystick + diamond only). The radial-wedge "no translational symmetry → non-looping" caveat stands; a looping offset would need the per-tier rigid-translation "tunnel" model (below).

...
- **Unlockable "repeating movements" — GREENLIT PLAN (Daniel, 2026-07-25).** The lock unblocks a class of default-locked live-driveable motions. Model Daniel set: **manual controls in canvas settings + matching canvas gestures**; autoplay / direction-lock / recenter are *fractional polish on top of a working core loop*, not core. Priority order + status:
  - **① Droste infinite zoom — ✅ SHIPPED B433 + FIXED B434** (looping slider `wireLoopingSlider` + pinch, direction corrected). **Remaining follow-ups:** (a) **mobile dedicated infinite-zoom SLIDER** — iPhone mobile-chrome has looping PINCH (shared output-gestures) but no jog-slider yet; needs its own touch wrapping-drag + gating (replace/hide mobile comp-zoom in droste via `applyFormVisibility`). (b) **smooth motion-tween** (add `drosteZoomPhase` to tween `CONTINUOUS_KEYS`) + **autoplay-wander** (add to drift; the include-override is the "lock a direction for autoplay" polish hook) — **mind the `FOLLOW_SPANS`-or-NaN gotcha: any new `CONTINUOUS_KEYS` member needs a `FOLLOW_SPANS` entry** (drosteZoomPhase span = 1). Linear tween is fine for natural 0→1 authoring; only 0.9→0.1 reverses. (c) **desktop-Safari check** — Safari trackpad pinch is GestureEvents, not the wheel+ctrl path; confirm it routes to phase (output-gestures touch/wheel covers most; input-bus pinch reroute covers mapped/gesture-event sources). (d) if the `+=`→`-=` direction still reads inverted on device, one-char flip in `foldDroste`.
  - **② Tiling canvas translation — NEXT, build the shared infra NOW (Daniel's call).** Pulls the canvas-translate foundation forward out of M6 (tile builder reuses it). Pieces: new form-agnostic `canvasOffset` state + shader `p -= canvasOffset` (today one-finger output drag is a no-op — this is the missing uniform, see [Canvas pan state] item); **per-form lattice vectors declared ON the form definitions** (building toward M4 geometry-truth, not against it) so the pan wraps at the true tile period — right→left, up/down, diagonal (pacman). **Mirror-group caveat:** for p4m/p6m/p3m1 the seamless period is the *translation lattice*, larger than the visible fundamental cell (translate by the wrong period → land on a reflected copy — same shape as the droste mirror ×2 finding). **Mini touch joystick** component (origin center, position dot wraps across the pad) + pan gesture. Rectangle first (cell = `squareAspect`); hex/triangle promote their hardcoded size constants to uniforms (the exact promotion M6 needs). New component → Lab.
  - **③ Offset + radial-wedge translation via the SAME joystick — LAST, gated on how ①+② feel.** Reuse ②'s mini joystick to drive droste center-offset and radial-wedge pan → translate-control **parity across all forms** (+ recenter). Neither loops (offset is a Möbius warp; wedge has no translational symmetry) — driveable, non-repeating. Only if it stays cheap; if either gets gnarly, stop and flag (a looping offset would need the per-tier rigid-translation "tunnel" model — see item below).

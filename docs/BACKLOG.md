...
- **Controls default to DROPDOWN MENUS (direction, Daniel 2026-07-06).** Output/broadcast and similar control clusters should live in dropdown menus by default, not in-flow expand bands — the camera dropdown (Build 233) is the first instance of the pattern. Applies to the Arc 1 save-menu/output rework and the Arc 2 panel control stacks; the `#outputRow`/`#canvasRow` band approach gets superseded as those surfaces are touched.
...
- **Form-picker location — OPEN DECISION before the Arc 2 panel rework.** Persisting above the source feels increasingly awkward. Options: (a) header control over the FORM SETTINGS stack (most logical — it determines which controls appear below it); (b) something global in the app bar (it has cascading impacts everywhere). Claude's lean: (a) — form choice is a per-slice authoring decision and motion mode locks it contextually, which a global control communicates poorly; revisit if perform mode needs fast form switching (a controller mapping serves that better than bar placement). **Align with Daniel before Arc 2 builds.**

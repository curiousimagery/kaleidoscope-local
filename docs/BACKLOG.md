...
- **Phase 3 — Tween/easing kit + still-animation (desktop footer).** **Tween primitive SHIPPED (Build 82, `src/kit/tween.js`).** Decisions settled (2026-06-02): A/B prove-out first → multi-keyframe later; discrete params locked per loop; loop = toggle that tweens end→start (no ping-pong). **A/B loop motion mode SHIPPED (Build 83):** motion-mode toggle → contextual footer (set/jump A & B, play/pause, loop, duration scrub, scrubber track); transient playback through the stateless engine (working state/undo untouched); gated to a still source, mutually exclusive with the live camera. **Still ahead:** generalize to a multi-keyframe model on the snapshot shape, draggable markers (uneven spacing) + scrubber previewing tweened states, aspect crop, and a video-export host module (WebCodecs preferred, MediaRecorder fallback; mp4/h264 if available else webm/vp9). Gated to larger viewports.

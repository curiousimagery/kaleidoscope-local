...
- **Phase 0 — Parameter registry (Kit).** *Registry shipped Build 65.* `src/shell/params.js` is the declarative catalog; the 6 clean sliders wire from it (loop over `DECLARATIVE_PARAM_IDS`), stateful/form-aware controls keep bespoke wiring with catalog-only entries. *Still pending:* the 30-min throwaway spike confirming the engine accepts `<video>` / canvas / `ImageBitmap` as a texture source (de-risks camera and video-file).

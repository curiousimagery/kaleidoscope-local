...
> - **Arc 5 — staged transitions. CORE SHIPPED B258 (hold · take · cut):** hold gates the follower's target feed (the stage edits off-air — discrete included), take commits the current stage at the shared transition speed and STAYS held (repeatable takes), cut snaps; the in-sync dot measures live-vs-stage divergence directly. **Remaining:** the **three-panel layout option** (Daniel: PiP is the right default but wants the option to switch into side-by-side panels; staged + live share perform mode — confirmed plan; the [PiP][three-panel] selector lives in the perform footer's RIGHT column per his directive), **the still-source footer strip** (his spec: current state on the LEFT, staged/upcoming state on the RIGHT, tween cells between, NO time ruler — needs engine captures of both states, the filmstrip capture machinery applies), **saved next-up snapshot slots** (a stage library beyond the single working stage), PiP auto-dock below a size threshold. **Watch item:** his CCW-rotation-followed-CW report (unreproducible post-B254; the lead cap likely fixed it — confirm at next validation). **Open keyboard question (space semantics):** space = stage/take shipped B259 since space is free in perform; if he later wants space back on play/pause for video sources, S/T remain the explicit pair — his call after feel-testing.

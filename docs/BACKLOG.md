...
- **Timeline thumbnails — "blue cells" in Safari only. Build 118 (FBO reuse) FAILED; Build 119 (fence sync) — verify.** Filmstrip/keyframe thumbnails (`engine.exportFrame` → FBO `readPixels` → `putImageData`) show partial/banded/channel-swapped (blue) cells on Safari that flicker + self-correct (repro on fresh webcam, so not image-specific). Diagnosed as `readPixels` reading before the GPU resolved the FBO (raw swizzled buffer → blue/banded); `gl.finish()` doesn't reliably block on Safari. Build 119 replaced it with a WebGL2 fence sync (`fenceSync`+`clientWaitSync`) before `readPixels`. **If still broken:** escape readPixels entirely — render→`drawImage` into a 2D canvas (the video-export path is immune). Wrinkle: that path resizes the live GL canvas, so thumbnails need a dedicated offscreen target (or keyframe thumbs `drawImage` a warmed-up preview canvas; the warm-up avoids the Build-110 first-add cold-sync lag).

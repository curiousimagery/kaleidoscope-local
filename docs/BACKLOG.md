...
- ~~**[BUG] Motion 16:9 default flickers in (B398).**~~ FIXED B409 — the output preview is hidden across the reshape (`body.motion-aspect-settling`) and revealed once 16:9 has painted. If a blank blink shows on device, layer in a skeleton box (deferred until seen).

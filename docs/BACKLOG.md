...
- **Point 5 — selectable 3-entity crossfade timeline SHIPPED B393** (tap left clip → drag its end/outT; tap right clip → drag its start/inT; tap crossfade band → drag its edges; band top-half, clips selectable below; freeze-then-reflow endpoint drag; live split-stage). Untested by Claude — feel-tuning expected.
...
- **Tiny-A-segment interaction** (from point 4): with honest proportions a 90/10 slice makes the right clip only ~10% of the track — grabbable but small. Daniel's zoom idea (focus a ~30s window, 15s either side of the seam) is the fallback if the proportional strip proves too tight. Revisit if the B393 interaction feels cramped.
- **Bounce bake — GOP-reverse buffering** (the deeper speedup). B393 put bounce on the WebCodecs reader (forward half fast) but the reverse half still re-decodes from the keyframe per frame (O(N²) within a GOP). The fix: when playing a GOP backward, decode it forward ONCE into a bounded buffer and serve in reverse, evicting per GOP. Bounds memory to one GOP; makes reverse ~linear. Lives in `video-decode.js`.

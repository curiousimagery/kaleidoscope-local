...
- The **active arc is Fold Live perform mode (Phase 1+2)** — see that section. Hardening/export work is a deliberate parallel lane, lower priority for now.
...
- **[LOW] Test-pattern button is inert during output-window broadcast.** The pattern is a BUS feature; the window self-renders and ignores it. Lean: **make the window honor it** (actually useful for projector/gallery alignment) — render the pattern when a `testPattern` flag is set. [Fold Live — Output window]
...
## Fold Live — perform mode & live output  ·  ACTIVE ARC

> **This is the home for the Fold Live perform-mode arc.** The durable spec is the plan `~/.claude/plans/in-our-last-thread-splendid-sparkle.md`; the **executable Phase 1+2 plan + kickoff** is `~/.claude/plans/fold-live-perform-phase-1-2.md` (start the next session there). This section is the rolling forward digest. **Phase 0** (Electron + Syphon output + output bus + diagnostics substrate) and the **enablement/hardening cluster** are DONE. Remaining roadmap (each phase ≈ one arc):
>
> - **▶ NEXT — Phase 1+2 (MUNGED) — perform-mode core.** Program/preview buses + take/transition (cut / smart-tween over N beats / auto on the next loop boundary), validated against the Arena hop + record-to-disk; lands on the stage's perform tier. **Pulls in only the motion-IxD cleanup perform reuses** (regroup controls by role; unambiguous loop on/off; single-select speed dropdown + 150%; space/delete). The deeper motion-editor redesign stays deferred (design-system arc). **This is the first real consumer of the design system.**
> - **Phase 3 — live gesture + smart-tween** (the deferred +gesture work, where it pays off most). Pairs with controller input (below).
> - **Phase 4 — single source as a ready asset** (clip editor callable from perform; baking-during-a-show is a flagged resource-contention question, not solved).
> - **Phase 5 — A/B crossfade** (the two-source engine blend — also the deck's engine; the engine only ever blends TWO sources).
> - **Phase 6 — multi-slot deck** = a library UI over the A/B engine (pre-paid by Phase 5).
> - **Later (native) — Capacitor / iOS HDMI**, after the Movink rig validates touch UX.

### ▶ NEXT — Perform-mode input (controller-driven, not gesture — settled direction; mapping TBD)
...
### ▶ NEXT — motion-IxD cleanup that perform mode reuses (Phase 1)

Scoped clean-as-you-go for perform mode (NOT the full motion-editor redesign, which stays deferred): regroup controls by role; **unambiguous loop on/off** [ties to design-system button disambiguation]; single-select **speed dropdown + 150%**; **keyboard** (space = play/pause, delete = delete selected). The "two control tiers" framing + exit criteria below guide it.
...
### ▶ NEXT — button-emphasis disambiguation (perform Phase 1 reuses it)
...
- **Preserve source across a chrome switch.** The responsive reload carries slice/canvas params but not the source image/camera (mobile↔desktop view switch still interrupts the source — tried before, NOT resolved). Persist the uploaded image (blob → IndexedDB) and re-`setSource` after reload; live camera re-prompts.
...
- **Export package layers / geometry overlay still.** Extend the package zip to include (a) the source thumbnail with the wedge overlay drawn on it, and (b) a **still showing the geometry treatment** (fold lines for the active form) rendered at output resolution as SVG or transparent PNG — the still parallel to the video's "how it was made" clip. Overlay math is in `overlay.js` (`drawSourceOverlay`); the lift is rendering geometry at export resolution + zip entries. Pairs with tile-aware export. Plus: ensure save-composition / save-package language is **consistent across mobile + desktop**.

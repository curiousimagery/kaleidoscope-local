...
- **[MEDIUM — new, Daniel 2026-07-08] Brave: FIRST video load in still mode shows a blank source panel.** Only the initial motion-footage load; picking another video loads fine; toggling still↔motion also corrects it. Likely the play-nudge-park + `paintSourceVideo` timing on Blink (the parked frame may not have presented when the panel painted). Reproduce in Brave, then check whether a delayed repaint after `loadeddata`/park fixes it.
...
> - **Arc 5 — staged transitions. CORE SHIPPED B258 (hold · take · cut):** hold gates the follower's target feed (the stage edits off-air — discrete included), take commits the current stage at the shared transition speed and STAYS held (repeatable takes), cut snaps; the in-sync dot measures live-vs-stage divergence directly. **Remaining:** the **three-panel layout option** (Daniel: PiP is the right default but wants the option to switch into side-by-side panels; staged + live share perform mode — confirmed plan; the [PiP][three-panel] selector lives in the perform footer's RIGHT column per his directive), **saved next-up snapshot slots** (a stage library beyond the single working stage), PiP auto-dock below a size threshold. **The still-source footer STRIP moved to the Parallel lane (Daniel lacks conviction, 2026-07-08):** spec = current state LEFT, staged/upcoming RIGHT, tween cells between, NO time ruler (engine captures of both states; filmstrip machinery applies; real perf/effort cost — NOT trivial, which is why the B261 placeholder shipped instead: source thumb + name · dims). Revisit later in the plan to see if it earns the effort. **Watch item:** his CCW-rotation-followed-CW report (unreproducible post-B254; the lead cap likely fixed it — confirm at next validation). **Open keyboard question (space semantics):** space = stage/take shipped B259 since space is free in perform; if he later wants space back on play/pause for video sources, S/T remain the explicit pair — his call after feel-testing.

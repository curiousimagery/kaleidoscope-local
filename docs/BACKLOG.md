...
- **[DIRECTED — Daniel 2026-07-08: pick up now, UNBLOCKED by the perform substrate] +gesture in the motion editor (gesture-record).** Record the continuous parameter stream while manipulating (perform proved every input path already feeds a single state stream — the follower's target feed IS the capture tap); smooth + simplify to sparse keyframes; detect return to the start ghost as the loop point (or end on release/press). The reserved [+gesture] footer slot is the entry. Sketch: press +gesture → armed → first manipulation starts sampling (state snapshots at ~30Hz) → press again (or a settle timeout) stops → simplify (Ramer-Douglas-Peucker over the normalized state distance from FOLLOW_SPANS) to sparse keyframes appended at the playhead. Ties to per-segment winding capture (above).

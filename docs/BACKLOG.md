...
- **Portrait source fill (crop-to-fit, adaptive).** Source view uses object-fit *contain*, so a portrait camera/source leaves big side gutters. Desired (Daniel): fill the panel width (crop toward square by default), revealing more of the true aspect as the source panel is stretched taller. Implementation: parameterize the shared overlay's fit (`contain` vs `cover`) — `drawSourceOverlay` computes imgX/Y/W/H for contain; add a `cover` mode (imgW = panel width, imgH = w/aspect, clipped by the already-`overflow:hidden` context) passed per-mount, desktop stays `contain`. The wedge UV↔screen mapping uses imgX/Y/W/H so it follows automatically.

...
- **Phase 0.5 — Camera host module + desktop/iPad wiring.** *Shipped Build 66.* Camera wired into the desktop chrome (`src/shell/camera.js` host module + continuous live loop in `main.js`); engine source generalized for `<video>`. Capture = freeze + save both (raw + kaleidoscope), stay editable; rear default + flip. *Possible follow-ups:* front-camera overlay/texture mirroring is preview-only today (handedness nuance — see CHANGELOG Build 66); a "save both" path that keeps the camera live for rapid capture was the non-chosen option if it's wanted later.

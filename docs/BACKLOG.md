...
- **Timeline + seam SHIPPED B388/B389/B390** (footage thumbnails + ruler; B→slice→A resequence; equal-half seam at true 50%; asymmetric crossfade region + draggable edge handles). **B390 review pass** fixed: no-load-playthrough (separate thumb video), action-named primary button, yellow crossfade band, scrub on every step (view-mapped), space plays crossfade step, "preview & bake" step shows trimmed resequenced loop. Untested by Claude — desktop-verify (VERIFY-QUEUE.md).
- ▶ **Undo/redo "still not working" (Daniel, B390) — NEEDS REPRO.** History core + wiring verified in isolation (trim/slice/crossfade/behavior all push; node harness passes); Daniel reports it still doesn't revert on Brave. Need: exact edit + step, and whether the top-bar undo button enables after the edit.
- ▶ **Split-stage seam-match: relocate or drop.** Retired from step 4 in B390 (couldn't coexist with the new play/scrub there). The two-frame "last-before | first-after" tool most naturally lives on the SLICE-POINT step (step 3), where dragging the cut changes the seam. Daniel's call.
- ▶ **Spit-and-polish UI refinement** (Daniel's stated next phase now that the structural work is done) — interaction-feel tuning across the stepped flow, the thumbnail timeline + seam drag.

...
- **[UX — Daniel 2026-07-29, ROOT-CAUSED] Locked controls give NO visible feedback on tap mid-broadcast.** The "aspect-lock unresponsive" was part user-error (aspect is an ENCODER-TIED hard lock during broadcast — intentionally gated; the fill-display toggle accommodates it) and part real trap: tapping a locked control does NOTHING visible — no toast — so it reads as broken. Root cause: `.m-locked` sets `pointer-events:none` on the control body, so the tap never hits a handler that could toast. Fix: a locked control should still capture the tap and surface a "locked while broadcasting — stop output to change" toast (a tap-catcher over the disabled control, or route the toast through the padlock). Also audit whether panel/popover overflow is eating the toast when it does fire.
- **▶ [DESIGN — Daniel 2026-07-29] Loosen mid-broadcast locks across platforms (feasibility done).** Daniel: "as a user trying to change the aspect myself, it feels reasonable to unlock → change → re-lock." Feasibility split by WHY each is locked: **(1) STRUCTURAL (form, segments, spiral, mirror, wedgeMirror, oobMode) — EASY to loosen:** they don't change output dims, just re-render from state; already modeled `unlockable:true` (form was unblocked B466). Make the rest unlockable across platforms; the guard is only against ACCIDENTAL change. **(2) ENCODER-TIED (frameAspect, outputRes) — feasible but with a STREAM-RESTART blip:** changing output dimensions mid-broadcast requires disposing + recreating the sender at the new size → NDI/Syphon source drops+reappears in Arena, HDMI window re-presents (same dispose+recreate caveat as live server rename). Offer an explicit "unlock → change (brief blip) → relock" flow IF Daniel accepts the blip; otherwise keep hard-locked + lean on fill-display for aspect. **Recommend:** loosen structural now (low-risk); decide encoder-tied with Daniel (blip tradeoff).
...
- **Zoom overflow cap `Z_SLICE_COVER` per-form calibration (Phase B).** B462 set it to a flat placeholder 3 ("covers source"); M4 Phase B should make it per-form (a small radial slice covers differently than a hex cell) alongside the other per-form default normalization below.
...
- **▶ NEXT DIAGNOSTIC (Daniel offered): USB-C ethernet dongle A/B.** Decisive — isolates WiFi-jitter from pipeline pacing. If ethernet is smooth → transport-bound → fix = an **adaptive publish-rate governor** (pace the publish to a STEADY sustainable rate — a smooth 30 beats a bursty 54 — and cap to the measured wire capacity) so WiFi is smooth-but-lower-fps instead of fast-but-stuttery. If ethernet STILL stutters → the pacing is in the pipeline (native drain / async send / rAF publish cadence), look there. Ethernet is a diagnostic only (not a production requirement).
- **Other levers:** fetch-POST unmasked transport (uncertain gain); chroma-subsample further / tile; even out the render→publish cadence.

### HDMI external-display follow-ups (core SHIPPED B331–B334)
- **✅ FIXED B465 (pending device verify) — iPad NATIVE HDMI detected the wrong display resolution.** Reported the sink as 2560×1440 for a 4K display + rendered the test pattern at that mode. Root cause: `FoldExternalDisplayPlugin` reported `bounds × scale`, which is the display's conservative default mode, not native. Now reports the largest `UIScreenMode` and promotes the screen to it before presenting, so the window + the state-rendering WKWebView (which draws both the program and the test pattern) are native-sized. No-op on AirPlay (already native).
- **▶ VIDEO SOURCES over the external display — genuinely INCOMPLETE (Daniel's #1 remaining gap, confirmed 2026-07-29).** Resolved the B465 ambiguity: motion-from-a-still + live-camera BOTH broadcast fine over HDMI/AirPlay (Daniel verified); the "video sources on the external display are coming — use a still or live camera for now" message fires ONLY for an actual loaded VIDEO source. So it's not a bug — it's unbuilt work. The blocker is the known **cross-webview blob problem** (below): the external WKWebView can't read the main context's `blob:` video URL. **Fix = write the clip to cache + serve it to the external view through the plugin's `fold-ext://` asset scheme, and sync its playback clock to the program.** This + the NDI stutter are the two biggest remaining conduit gaps. Non-trivial (native asset serving + video-clock sync across webviews); scope as its own build, not a quick fix.

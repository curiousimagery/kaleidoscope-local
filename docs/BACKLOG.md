...
- **Add a 150% speed preset** (speeding up never needs interpolation — free now).
- **Tier-1 source-fps hint** — show est. source fps + warn when a preset drops effective fps below ~15. Cheap interim before frame interpolation.
...
- **Timeline + seam SHIPPED B388–B391** (footage thumbnails + ruler; B→slice→A resequence; equal-half seam at true 50%; asymmetric crossfade region + draggable edge handles; undo/redo of trim/slice/crossfade/behavior — Daniel-confirmed working). **B390** review pass: no-load-playthrough (separate thumb video), action-named primary button, yellow crossfade band, scrub on every step (view-mapped), space plays crossfade step, "preview & bake" step shows trimmed resequenced loop. **B391**: two-frame seam-match shows WHILE dragging a crossfade edge; scrubbing the crossfade zone previews the real dissolve (B→A blend). Untested by Claude — desktop-verify (VERIFY-QUEUE.md).
- ▶ **NEXT — point 5: the selectable 3-entity crossfade timeline** (Daniel's review, checkpointed B392). On the crossfade step, make the left clip / crossfade / right clip separately selectable: click the left clip → drag its endpoint (= trim.outT, B's last-before-seam frame); click the right clip → drag its startpoint (= trim.inT, A's first-after-seam frame); click the crossfade → drag its edges (duration). Daniel's layout idea: crossfade band in the TOP half of the track, clip bodies selectable in the BOTTOM half (crossfade always reachable above, clips always reachable below no matter the crossfade width). Re-enables the split-stage as a LIVE seam-match shown WHILE dragging an endpoint. Proposed model: proportional strip (shipped B392); selection state (B/xfade/A); frozen-scale endpoint drag with reflow on release; value overlay on drag.
...
- **Tiny-A-segment interaction** (from point 4): with honest proportions a 90/10 slice makes the A clip only ~10% of the track — grabbable but small. Daniel's zoom idea (focus a ~30s window, 15s either side of the seam) is the fallback if the proportional strip proves too tight to interact with. Revisit alongside point 5.
...
- **Frame interpolation — wanted sooner (Daniel confirmed).** At 30fps source, 25% is unusable, 50% is a bit choppy. Interpolation (blend first, optical-flow later) unblocks sub-25% speeds; can bake. Cheap interim: the Tier-1 source-fps hint + add 150% now.

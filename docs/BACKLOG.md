...
- **[BUG — Daniel field test 2026-07-28, NEEDS ON-DEVICE INSPECTION] iPhone HDMI: the ASPECT-LOCK unlock is totally unresponsive.** On the mobile chrome with HDMI out, unlocking the aspect lock is dead (locking works). Investigated B465 and the obvious suspects all CHECK OUT: the padlock is built by the shared `makeLockToggle` (has the `lock-toggle` class the tap-arbitration bail exempts), it's appended to the `.m-control-row`, and `.m-locked` disables pointer-events only on `> :not(.m-control-row)` — so the padlock's row is explicitly exempt and should stay tappable, same as the (working) segment lock. No code-level difference found. **Need on-device (Safari Web Inspector on the iPhone):** what actually eats the tap — the canvas POPOVER's tap-outside/dismiss handler (aspect lives in the canvas popover; segments does not), a z-index/overlay from the HDMI-connected state, or the padlock reporting `contextual`/non-unlockable while live. Repro with the inspector open + a breakpoint on the lock toggle's handler.
...
The native drain is idle; frames arrive 50ms apart. WebKit's WS send moves 8.3MB/frame at ~165MB/s, so FHD RGBA delivers ~20fps. **Lever (a) SHIPPED B465: UYVY 4:2:2 is now the default wire** — halves the bytes (FHD now fits ~30fps, HD ~60fps under the wall), and the blue cast that had kept it opt-in is fixed at the capture layer. **Remaining levers if it's still not enough after verify:** (b) fetch-POST unmasked transport (uncertain gain); (c) USB-C ethernet is the rig answer; (d) chroma-subsample further / tile. **Verify at Gate 1** whether UYVY closes the gap or ethernet is still needed.
...
- **[BUG — Daniel field test 2026-07-28, NEEDS REPRO DETAIL] External-display "video sources coming" when playing MOTION FROM A STILL.** Investigated B465: the gate (`external-display.js` `buildSourcePayload`) keys on `env.sourceVideo` (the live `<video>` element), which is **null for a still source** — so it should NOT fire for a still-source motion. Either the session had a video loaded (stale `sourceVideoUrl`), or the symptom is the external view showing the still STATICALLY (motion state not streamed post-connect) rather than an actual error. **Need on-device:** does it literally throw the message with a fresh still, or show a static image? And is a state-stream to the external WKWebView expected during motion? (If motion-from-still should animate the external display, that's a feature gap, not the gate.)

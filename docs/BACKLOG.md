...
> - **Arc 5 — staged transitions (commit semantics; the panel moved into Arc 4).** Save a next-up snapshot, edit off-air, **blend in** on the shared speed control while output-live keeps the old state. PiP-minimize polish (explicit control + auto-dock below a size threshold).

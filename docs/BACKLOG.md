...
- **Infinite zoom (seamless loop) — ▶ THIS ARC — LOOP MATH LANDED B432, control surface OPEN.** Animate composition zoom across exactly one factor-of-`drosteZoom` interval: `foldDroste` reduces log-radius mod `log(drosteZoom)`, so the field is periodic under radial scaling by `drosteZoom` — full-zoom-in ↔ full-zoom-out loops with no visual change. **`kit/droste-zoom.js` (B432) = the pure, driver-agnostic math** (period, multiplicative wrap, per-frame stepper). Two seamlessness preconditions BAKED IN: (a) **offset centered** — the Möbius pre-comp is not scale-invariant (default-locked offset gives us this); (b) **spiral = 0** for a *pure* zoom — spiral keeps the radial shift exact (`c.real=1`) but leaves a residual source rotation per loop, so a spiral zoom needs canvasRotation coupled in lockstep (the Droste screw motion — `spiralResidualPerLoop` exposes the amount, NOT wired, needs on-device sign check). MIRROR tiers double the period (×zoom²). **OPEN: the control surface** (auto-toggle vs. an autoplay field vs. a touch joystick) — decided with the repeating-movements proposal below. Shares the snap-point idea with tile snap-to-zoom.
- **Unlockable "repeating movements" — the lock unblocks a whole class (Daniel, 2026-07-25).** Now that per-control locks exist, controls that were too easy to trip by accident can become *default-locked* live-driveable motions. The family, by seamlessness: **(1) Droste infinite zoom** — periodic (above), seamless. **(2) Tiling pan (rectangle/hex/triangle)** — translation is periodic under the tile lattice, so panning X/Y wraps seamlessly (right edge → left) on a *centered canvas*; the natural parallel to zoom. Needs a `canvasOffset` translate state + shader uniform (see [Canvas pan state] item) that respects the lattice period per form. **(3) Droste center-offset "pan"** — Daniel's idea of looping the offset right→left. CAVEAT: the offset is a *Möbius* parameter, NOT a lattice translation — it does **not** wrap seamlessly the way (2) does; "looping back to the left" would pop unless reframed. Feasibility TBD (may want the per-tier rigid-translation "tunnel" offset model instead — see item below). **(4) Radial wedge translate** — pan across the source within the wedge; **does not repeat** (no translational symmetry), Daniel already flags this. Shared control idea: an **on-canvas / touch joystick** driving X/Y (or single-axis zoom), default-locked, with a **"recenter" affordance** always available. Proposal + open Qs going to Daniel this pass.

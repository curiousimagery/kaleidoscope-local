...
- **Default OOB → mirror (not clamp).** Mirror is generally the preferred out-of-bounds mode; make it the default so it isn't an extra step every session. One-line default change (`state.oobMode = 1`) plus the active-button sync (and the mobile OOB control).

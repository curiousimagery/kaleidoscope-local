...
> - **Arc 2 — panel-sibling overhaul.** Source/output become symmetric, middle-aligned, divider-repositionable siblings (built third-panel-ready). Placement RESOLVED (Daniel, 2026-07-07, after living with the below/flank variations — neither worked): **progressive disclosure** — each panel persists its essentials beneath the visual (source: scrubber/meta/form-picker/reset-slice; output: **frame aspect** + reset canvas) and the dense controls open in a **popover** from a trigger beside the selectors (shipped B243). Still remaining in-arc: source fit/fill (from mobile) + **aspect expansion:** 1:1 · 5:4 · 4:3 · 3:2 · 16:9, landscape default, click-again flips portrait, swap affordance on hover.

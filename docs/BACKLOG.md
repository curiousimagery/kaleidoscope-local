...
- **Droste: vanishing-point offset (Build 44).** Möbius pre-composition `M(p) = (p − a) / (1 − conj(a)·p)` applied before the log warp; moves the spiral singularity off the geometric center for PhotoSpiralysis-style eccentric spirals. Design approved in the staged plan: small drag handle (~14 px hit) at slice center, with the area between center and inner ring + the annulus body taking over the existing `'move'` gesture. State: `drosteOffsetX`, `drosteOffsetY` (clamped `|a| ≤ 0.95`). Will reorganize Droste's `classifyPointer` priority order. Note: the log-shear math from Build 43 still composes cleanly with the Möbius pre-comp.

- **Droste: pole rotation (Build 45).** Second Möbius parameter (post-composition) controlling where infinity maps to — the "rotate the Riemann sphere" feel. UX likely a second handle outside the outer ring at a representative position for the anti-pole. Detailed design deferred to start of Build 45, once iPad time with the Build 44 vanishing-point handle has informed the affordance pattern. Strong pairing with the motion shell — animating the pole over time gives a flowing-water effect.

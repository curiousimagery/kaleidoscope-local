...
- **+gesture SHIPPED B267 (core).** Open tails: winding capture (a recorded 720° spin currently collapses to per-segment shortest-path on playback — ties to the per-segment winding item above); a possible APPEND mode (v1 replaces the timeline — Daniel to react); return-to-start-ghost auto-stop (v1 stops on stillness/press; the closure check only trims the closing sample); gesture INTO an existing loop = the stage-changes/ownership pattern later.

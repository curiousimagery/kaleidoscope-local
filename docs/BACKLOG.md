...
- **[HIGH, ARC 0] Undo doesn't register keyframe interactions.** Add/move/delete/retime of keyframes never enter the history stack — undo after timeline work jumps back past it unpredictably. Integrate keyframe ops into `shell/history.js` (decide snapshot scope: `motion` lives beside `state`).
...
- **[HIGH, ARC 0] Phone PWA: saving a still/composition leaves the app, and returning loses the output preview.** **Consistent repro on the current build (Daniel, 2026-07-06).** The save/share hands off to iOS; on return the WebGL preview is blank — almost certainly **WebGL context loss while backgrounded** with no restore-repaint. Fix = the **WebGL context loss/restore** handlers (`webglcontextlost`/`restored` on the preview canvas → re-init GL + repaint current `state`). Mobile-chrome save flow.
...
- **[MED] Test pattern doesn't work on iPad.** The output test pattern fails to render/publish on iPad; diagnose which stage (bus render vs sink) breaks under iPadOS Safari.
...
- **Source/live-status overlay clutters the preview.** ~~Move it into the bar / the source (left) group~~ **Resolved by the Arc 1 spec:** filename lives under the upload button; the selected camera name under the camera button. Implement as part of Arc 1.

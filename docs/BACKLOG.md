...
- **Phase 0.5 — Camera host module + desktop/iPad wiring.** *Shipped Build 66, fixed Build 67 after iPad testing.* Camera wired into the desktop chrome (`src/shell/camera.js` host module + continuous live loop in `main.js`); engine source generalized for `<video>`/`<canvas>`. Front camera feeds a mirrored frame so sampling matches the preview. Capture = freeze as editable still (no auto-save); first export saves the unmodified original + kaleidoscope, later exports just the kaleidoscope. Desktop defaults to front (mirrored), iPad to rear; flip control. *Possible follow-ups:* custom multi-camera device picker (Firefox shows one, Safari forces default); a rapid-capture mode that keeps the camera live; two-file export may hit browser "multiple downloads" prompts.

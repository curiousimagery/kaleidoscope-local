...
- **Unify the control registries so new controls auto-map (Daniel-raised 2026-07-25) — ▶ PROPOSE-FIRST.** Two hand-maintained lists exist: `PARAMS` (params.js — UI controls, ranges, fmt/parse, form gating) and `PARAM_TARGETS` (input-bus.js — the MIDI/gamepad/remote mapping catalog with min/max/wrap/dir). They overlap but are separate, so a new control can ship in the UI yet be silently **unmappable** to controllers (exactly what happened: pan wasn't mappable until B439 added it by hand; infinite zoom only was because the pinch reroute added it). **Goal:** one source of truth so any new adjustable control auto-surfaces as a mapping target. Sketch: give `PARAMS` entries the mapping metadata (min/max/wrap/wrapPeriod/dir/mode-hints) and derive `PARAM_TARGETS` from it (filtering to the mappable ones); form-routed keys (segments→drosteArms) and the droste-in-context swaps (infinite zoom replaces canvas zoom) need a resolver. **Also:** discrete controls (segment count, form) aren't mappable yet — they need **snap-aware** mapping (continuous input → nearest valid integer/enum) since `writeParam` writes raw values. **Conduit lens:** the input bus + this registry are prime shared-infra candidates (every consumer app maps controllers to params). Propose the unified shape before building.

...
> - **Arc 5 — staged transitions. CORE SHIPPED B258 (hold · take · cut):** hold gates the follower's target feed (the stage edits off-air — discrete included), take commits the current stage at the shared transition speed and STAYS held (repeatable takes), cut snaps; the in-sync dot measures live-vs-stage divergence directly. **Remaining:** the **three-panel layout option** (Daniel: PiP is the right default but wants the option to switch into side-by-side panels; staged + live share perform mode — confirmed plan), **saved next-up snapshot slots** (a stage library beyond the single working stage), PiP auto-dock below a size threshold.

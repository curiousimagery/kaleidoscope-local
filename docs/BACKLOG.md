...
**Layering vocabulary (settled).** "Shell" was conflating four layers with different reuse profiles, which made each use case sound like a rebuild. It isn't:

- **Engine** (`src/engine/`) — forms, shader, gl, geometry. Pure pixels, zero DOM. Never rebuilt.
- **Kit** — DOM-agnostic interaction primitives shared by every front-end: state schema (`shell/state.js`), undo/redo (`shell/history.js`), overlay hit-test math, plus the parameter registry, tween/easing primitive, and keyframe model still to build. Grows over time; never rebuilt.
- **Host** — what feeds state in / takes pixels out: input source (file / camera / video), render driver (on-demand / continuous loop), output sink (still FBO / video export). Composable modules, *assembled* per use case, not rebuilt.
- **Front-end / chrome** — the layout + widgets for a device. The only layer genuinely rebuilt per context — and even its differences are mostly *which registry parameters and host modules it wires*.

**Two front-ends, plus a deferred third:** the **desktop chrome** (current, extended — serves desktop still, still animation, video animation, and live camera on iPad) and the **mobile chrome** (new — camera-first, touch; serves mobile still editor, mobile camera, live capture-to-disk). Camera is a *host module* wired into both, not its own chrome. The keyframe/timeline editor is desktop/iPad only. A live MIDI/kiosk VJ surface is a possible third, shape TBD.
...
- **Phase 1 — Mobile still-editor chrome.** Second front-end, renders the Phase 0 registry as touch-first progressive-disclosure controls. No new engine capability — pure re-presentation. **Opens with a divergent IxD exploration session Daniel drives** (2–3 layout/flow approaches; see "mobile UX exploration notes" below). Reuses engine, state schema, history, touch-aware overlay, export path; does not reuse the desktop split-pane / divider / scrub-field / right-panel chrome.
- **Phase 2 — Camera through the mobile chrome.** Wires the Phase 0.5 host module into mobile; camera-first as default entry, photo-import as secondary. Non-destructive re-edit drops the captured raw frame into the Phase 1 editor. Cheap once both exist.
...
- **Deferred: PWA manifest + offline cache.** Proper `manifest.json` and service worker so the app works offline once loaded. iPad install-to-homescreen story. Plumbing for the iPad-app-via-Capacitor path (see `FOLD.md` monetization Phase 3).

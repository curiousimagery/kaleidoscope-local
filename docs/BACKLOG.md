...
- **Phase 0 — Parameter registry (Kit).** Promote hand-wired control definitions (range/step/label/snap/scrub) out of `controls.js` into a declarative `src/shell/params.js` that all chromes render from. `wireSliderWithScrub()` reads its opts from the registry; desktop behavior unchanged. Plus a 30-min spike confirming the engine accepts `<video>` / canvas / `ImageBitmap` as a texture source (de-risks camera and video-file). Prerequisite for the mobile chrome.
- **Phase 0.5 — Camera host module + desktop/iPad wiring.** `getUserMedia` → continuous render driver → camera-as-source → dual capture (kaleidoscope output at full FBO resolution + raw camera frame at native resolution). Shutter UI; wedge overlay drawn on the live camera view, draggable on touch. iPad runs the desktop chrome and is an excellent capture surface (direct manipulation, portable camera, large screen), so this delivers the camera moment early. Independent of the mobile chrome; parallelizable with Phase 1.

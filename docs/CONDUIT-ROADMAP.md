...
- **External surface (iOS external display / AirPlay + desktop output window), UNIFIED**: these are the same shape today, split across `output-window.js` / `external-display.js` / `output-view.js`. Fold renders the second surface from committed STATE (zero readback — why it flies at 4K). The conduit-able piece is the **transport-neutral PLUMBING** — secondary-surface lifecycle (UIScreen watch / desktop window / the Capacitor webview), the message channel, the hello/fps handshake, context-loss recovery — with a **consumer-provided view** that render-from-state (the recommended, only-shipped pattern; `output-view.js` is the reference). Frame-push stays possible as a consumer choice over the same channel with NO conduit fallback to maintain. Design doc first, then a contained extraction.

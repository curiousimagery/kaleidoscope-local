...
- **📱 iPhone record-video take has AUDIO (B410, needs `npm run cap:sync` + Xcode rebuild).** Record a video-mode take on the iPhone → the saved file should now have sound. Native fix: the video capture session no longer reconfigures the app audio session (was silencing the getUserMedia mic). If it's STILL silent, open the console for the `[fold] record mic:` line at record start — `NONE` = track never acquired (permission/acquisition), `enabled=true muted=false state=live` = track was fine so the issue is encoding/session-level (report the line either way). iPhone specifically (iPad record + mic was already confirmed).
- **📱 Mobile fills 100% width again — no horizontal scroll (B411).** On the iPhone (and any phone), the app should fill the width with no horizontal scrollbar and no clipped/broken layout. The 600px desktop floor is now `@media (pointer: fine)` only. Confirm the iPhone UI is back to normal.
- **🖥️ STILL OPEN — motion 16:9 square-first (B411 diagnostic).** B410's sync-render didn't fix it. On desktop Firefox, open the console, enter motion, and report the `[fold] motion-enter aspect: willChange=… defaulted=… frameAspect=…` line (plus: is it the first motion entry after reload? video or image source?). That tells me whether the hide path runs; I'll fix from there and remove the log.

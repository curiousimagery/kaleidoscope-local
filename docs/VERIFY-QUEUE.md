...
- **🖥️📱 Motion 16:9 enters with no square-first transition (B410, supersedes the B409 check).** Enter motion with a non-16:9 (e.g. square) source → the first motion frame should already be 16:9 landscape, with no square-then-landscape step. B409 hid the reshape but revealed a beat too early; B410 paints 16:9 synchronously before revealing. If you STILL see a square first, tell me: was it the first motion entry after load, and what aspect was the source/still preview beforehand?

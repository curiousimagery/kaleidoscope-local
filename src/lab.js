...
const LINK_PARAMS = [
  { group: 'Chrome + panels  ·  the ones you reach for', items: [
    ...
    ['?tone=50,1', 'sweep the HDR tone curve — shoulder, then exposure',
      'HDR sources (HLG/PQ) get tone mapped to SDR, and the curve is a LOOK rather than a correctness question, so it is tunable instead of guessed. First number is the Reinhard shoulder (white point SQUARED): LARGER is softer highlights. 16 is almost linear at the top and blows highlights; 50 is the default; 120 is close to pure Reinhard. Second number is exposure, for when the whole image is off rather than just its highlights. The reference to tune AGAINST is the Loop Builder, whose stills come from AVAssetImageGenerator and therefore get Apple\'s own HDR-to-SDR rendering. Seeds the value; since B763 the frame-cost panel has live steppers for it (shown only when an HDR source is loaded), and the swept value rides `copy report` as `tone.url` ready to paste back. See engine/color.js TONE_DEFAULTS.'],

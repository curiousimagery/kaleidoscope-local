...
const LINK_PARAMS = [
  { group: 'Chrome + panels  ·  the ones you reach for', items: [
    ...
    ['?tone=1,1,1', 'HDR tone curve: shoulder, exposure, gamma',
      'Seeds the three-stage HDR-to-SDR curve; since B763 the frame-cost panel has live steppers for it (HDR sources only), and the swept value rides `copy report` as `tone.url` ready to paste back. APPLIED IN THIS ORDER, which is also the order to sweep them: exposure (linear gain on everything), then gamma (bends midtones, leaves black and white fixed), then shoulder (highlight roll-off, expressed as the white point SQUARED, and 1.0 means OFF). The defaults are all 1, i.e. no curve at all — HLG is SDR-backward-compatible by design (BT.2100), so decoding and displaying it is the correct starting point, and B761/B762 dividing by its reference white is what blew the highlights out. The reference to match is the Loop Builder, whose stills come from AVAssetImageGenerator and therefore get Apple\'s own rendering. See engine/color.js TONE_DEFAULTS.'],

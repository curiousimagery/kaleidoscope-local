...
export const Z_SLICE_MIN = 0.05, Z_SLICE_COVER = 3, Z_CANVAS_MIN = 0.05, Z_CANVAS_MAX = 4;
...
export function applyUnifiedZoom(state, factor) {
  ...
  if (factor >= 1) {                                                   // ZOOM IN
    ...
    else if (s > Z_SLICE_MIN)   s = Math.max(Z_SLICE_MIN, s / factor);  // overflow past the wall: zoom into source detail
  } else {                                                             // ZOOM OUT
